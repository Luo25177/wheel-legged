...
#define Isempty(T) \
	int isempty_##T(queue_##T* queue) { return queue->head == queue->tail; }
#define Isfull(T) \
	int isfull_##T(queue_##T* queue) { return queue->tail == (queue->head - 1 >= 0 ? queue->head - 1 : queue->head - 1 + queue->size); }
#define Push(T)                                                                                  \
	void push_##T(queue_##T* queue, T val) {                                                       \
		if (queue->tail == (queue->head - 1 >= 0 ? queue->head - 1 : queue->head - 1 + queue->size)) \
			return;                                                                                    \
...
#define Pop(T)                             \
	bool pop_##T(queue_##T* queue, T* val) { \
...
#define DEFINEQUEUE(T) Maxsize(T) Getsize(T) Isempty(T) Isfull(T) Push(T) Pop(T) Clear(T) NewQueue(T) DeleteQueue(T)
