...
void         jumpMode() {
  switch (car.jumpPhase) {
    ...
    case KICK:
      ...
      break;
    case SHRINK:
      car.legL.L0pid.target = 0.32;
      car.legR.L0pid.target = 0.32;
      if (car.force > 100)
        car.jumpPhase++;
      break;
    case BUFFER:
...
void LTD(float ref, DataRefer* trace) {
  float now  = trace->now + dt * trace->dot;
  trace->dot = trace->dot + dt * (-LTDBANDWITH * LTDBANDWITH * trace->now - 2 * LTDBANDWITH * trace->dot + LTDBANDWITH * LTDBANDWITH * ref);
  trace->now = now;
}
